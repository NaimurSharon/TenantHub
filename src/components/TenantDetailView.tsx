...
export function TenantDetailView({ customerId, isTablet, onBack }: TenantDetailViewProps) {
  ...
  const currentDisplayName = hubData?.header?.display_name ?? tenant?.name ?? "Tenant Detail";
    ...
    <View style={styles.container}>
      ...
      {!isTablet && onBack && (
        <View style={styles.mobileHeader}>
          ...
          <Text style={styles.mobileHeaderTitle}>{currentDisplayName}</Text>
          <View style={{ width: 28 }} />
        </View>
      )}
      ...
      >
        ...
        {activeTab === "Profile" && (tenant || hubData) && (
          <ProfileTab tenant={tenant ?? hubData?.header} summary={hubData?.summary} />
        )}
      </ScrollView>
    </View>
  );
